...
/**
 * AssemblyAISTTWebSocketNode processes continuous multimodal streams (audio and text)
 * using Assembly.AI's streaming Speech-to-Text service via direct WebSocket connection.
 *
 * This node:
 * - Handles both audio and text input from multimodal streams
 * - For text input: bypasses STT and returns immediately with the text
 * - For audio input:
 ...
 * - Returns DataStreamWithMetadata with transcribed text and silence metrics when a turn completes
 */
export class AssemblyAISTTWebSocketNode extends CustomNode {
  ...
  /**
   * Process multimodal stream (audio or text) and transcribe using Assembly.AI WebSocket
   */
  async process(
    ...
    try {
      ...
      // Process multimodal stream (audio or text) and send to Assembly.AI
      const audioProcessingPromise = (async () => {
        try {
          console.log(
            `[AssemblyAI WS STT - Iteration ${iteration}] Starting multimodal stream processing loop`,
          );
          ...
          while (true) {
            ...
            if (session.shouldStopProcessing) {
              console.log(
                `[AssemblyAI WS STT - Iteration ${iteration}] Session closed externally - stopping processing`,
              );
              break;
            }
            ...
            if (result.done) {
              console.log(
                `[AssemblyAI WS STT - Iteration ${iteration}] Stream exhausted after ${audioChunkCount} chunks`,
              );
            ...
            if (content.text !== undefined && content.text !== null) {
              console.log(
                `[${new Date().toISOString()}] [AssemblyAI WS STT - Iteration ${iteration}] Text content detected: "${content.text}"`,
              );
              isTextInput = true;
            ...
            // Otherwise, this is audio content - process it
            if (content.audio !== undefined && content.audio !== null) {
              ...
              const chunkDurationMs =
                (float32Data.length / audioSampleRate) * 1000;
      ...
      console.log(
        `[${new Date().toISOString()}] [AssemblyAI WS STT - Iteration ${iteration}] Returning DataStreamWithMetadata (is_running: ${!isStreamExhausted}, is_text_input: ${isTextInput}, transcript: "${transcriptText}", endpointing latency: ${endpointingLatency.toFixed(0)}ms)`,
      );
  ...
  private sendPartialTranscript(
    ...
    try {
      // Create text event with final=false for partial transcripts
      const textEvent = EventFactory.text(
        text,
        interactionId,
        { isUser: true },
        false,

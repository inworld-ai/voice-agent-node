...
export class RealtimeSessionManager {
  ...
  async updateSession(event: RT.SessionUpdateEvent): Promise<void> {
    ...
    // Normalize model fields: accept both camelCase and snake_case for compatibility
    const resolvedModelId =
      sessionConfig.modelId ?? (typeof sessionConfigAny.model_id === 'string' ? sessionConfigAny.model_id : undefined);
    const resolvedModelSelection =
      sessionConfig.modelSelection ?? (sessionConfigAny.model_selection as RT.LLMSelectionConfig | undefined);
    ...
    if (sessionConfig.audio) {
      ...
      if (sessionConfig.audio.output) {
        ...
        if (sessionConfig.audio.output.model !== undefined) {
          const modelValue =
            typeof sessionConfig.audio.output.model === 'string'
              ? sessionConfig.audio.output.model
              : String(sessionConfig.audio.output.model);
          this.realtimeSession.session.audio.output.model = modelValue;

          // Store TTS model in connection state for TTS node
          const connection = this.realtimeApp.connections[this.sessionKey];
          if (connection) {
            connection.state.ttsModelId = modelValue;
            logger.info({ sessionId: this.sessionKey, ttsModelId: modelValue }, `Updated TTS model to ${modelValue}`);
          }
        }
    ...
    if (sessionConfig.tools !== undefined) {
      ...
      // Update connection state with tools (State uses runtime Tool; contract Tool is compatible at runtime)
      const connection = this.realtimeApp.connections[this.sessionKey];
      if (connection) {
        connection.state.tools = sessionConfig.tools as State['tools'];
      }
    }
    ...
    if (sessionConfig.toolChoice !== undefined) {
      ...
      // Update connection state with toolChoice (State uses runtime ToolChoice; contract is compatible at runtime)
      const connection = this.realtimeApp.connections[this.sessionKey];
      if (connection) {
        connection.state.toolChoice = sessionConfig.toolChoice as State['toolChoice'];
      }
    }
    ...
    if (resolvedModelId !== undefined) {
      this.realtimeSession.session.modelId = resolvedModelId;

      // Update connection state
      const connection = this.realtimeApp.connections[this.sessionKey];
      if (connection) {
        connection.state.modelId = resolvedModelId;
        logger.info({ sessionId: this.sessionKey, modelId: resolvedModelId }, `Updated connection.state.modelId`);
      }
    }

    if (resolvedModelSelection !== undefined) {
      this.realtimeSession.session.modelSelection = resolvedModelSelection;

      // Update connection state
      const connection = this.realtimeApp.connections[this.sessionKey];
      if (connection) {
        connection.state.modelSelection = resolvedModelSelection;
        logger.info(
          { sessionId: this.sessionKey, modelSelection: resolvedModelSelection },
    ...
    if (sessionConfig.textGenerationConfig !== undefined) {
      ...
      const connection = this.realtimeApp.connections[this.sessionKey];
      if (connection) {
        connection.state.textGenerationConfig = sessionConfig.textGenerationConfig as any;
        logger.info(

...
export class TextInputNode extends CustomNode {
  ...
  process(context: ProcessContext, input: TextInput): State {
    logger.debug(
      { sessionId: input.sessionId, text: input.text?.substring(0, 100) },
    ...
    if (text === TOOL_CALL_CONTINUATION_MARKER) {
      logger.info(
        ...
        'TextInputNode: Tool call continuation - skipping user message, continuing with existing conversation',
      );

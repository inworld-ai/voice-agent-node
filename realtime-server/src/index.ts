 ...
 *
 * //	Sec-WebSocket-Protocol: bearer_eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
 * //	Sec-WebSocket-Protocol: basic_dXNlcm5hbWU6cGFzc3dvcmQ=
 *
 * 1. w-proxy basic sec-websocket-protocol
 *  basic INWORLD_API_KEY - w-proxy passthrough
 * 2. w-proxy bearer sec-websocket-protocol JWT
 *  generated JWT token - w-proxy passthrough
 */
function extractInworldApiKey(headers: http.IncomingHttpHeaders): string | undefined {
  ...
  /**
   * Basic token - w-proxy passthrough as it is.
   * Bearer token - to w-proxy passthrough as basic token if the feature flag is enable:
   * https://unleash.inworld.ai/projects/default/features/w-proxy.convert-jwt-to-basic-auth
   */

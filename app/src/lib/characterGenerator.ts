...
// Default LLM configuration for character generation
const DEFAULT_LLM_MODEL_NAME = process.env.CHARACTER_GENERATION_LLM_MODEL_NAME || 'llama-3.3-70b-versatile';
const DEFAULT_PROVIDER = process.env.CHARACTER_GENERATION_LLM_PROVIDER || 'groq';
import { getCharacterGenerationPrompt, VALID_VOICE_IDS } from './prompts/characterPrompts';
...
async function initCharacterGenerationGraph(apiKey: string): Promise<Graph> {
  ...
  const llmNode = new RemoteLLMChatNode({
    ...
    provider: process.env.CHARACTER_GENERATION_LLM_PROVIDER || DEFAULT_PROVIDER,
    modelName: process.env.CHARACTER_GENERATION_LLM_MODEL_NAME || DEFAULT_LLM_MODEL_NAME,
    stream: false,
